...
usage() {
    cat <<HELP
Usage: bash install.sh [--version v0.5.0rc1|latest] [--profile recommended|core] [--extras name[,name]]

Environment equivalents:
  OPENSQUILLA_VERSION=v0.5.0rc1
...
if [[ "${release_selector}" != "latest" && "${release_selector}" != "stable" ]] && ! is_release_version "${release_selector}"; then
    ...
    echo "install.sh: the release installer only supports latest, stable, or release versions like v0.5.0rc1." >&2
