...
if ($Version -notin @('latest', 'stable') -and -not (Test-ReleaseVersion $Version)) {
    Write-Error "install.ps1: unsupported OPENSQUILLA_VERSION='$Version'. The release installer only supports latest, stable, or release versions like v0.5.0rc1. Use git clone plus scripts/install_source.ps1 for main, dev, branch, or source installs."
    exit 1
}
